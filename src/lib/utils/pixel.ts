...
export function drawWaveform(ctx: CanvasRenderingContext2D, x: number, y: number, frame: number) {
	...
	for (let i = 0; i < 48; i++) {
		const phase = (i + frame * 3) * 0.2;
		const height = Math.floor(Math.sin(phase) * 6 + Math.cos(phase * 0.7) * 2);
		const py = midY - height;
		ctx.fillRect(x + i, py, 1, Math.abs(height) + 1);
	}
}
...
export function drawMiniWaveform(
	...
	for (let i = 0; i < width; i++) {
		const phase = (i + frame * 2) * 0.3;
		const height = Math.floor(Math.sin(phase) * 3 + Math.cos(phase * 0.5) * 1);
		ctx.fillRect(x + i, midY - height, 1, Math.abs(height) + 1);
	}
}
